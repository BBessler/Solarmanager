...
REPO_RAW="https://raw.githubusercontent.com/BBessler/Solarmanager/main"

# Shared Library laden
LIB_DIR="$(dirname "$0")"
if [ ! -f "$LIB_DIR/lib_solarmanager.sh" ]; then
    curl -fsSL "$REPO_RAW/install/lib_solarmanager.sh" \
...
GRANT ALL PRIVILEGES ON *.* TO 'root'@'%' IDENTIFIED VIA mysql_native_password USING PASSWORD('$db_password') WITH GRANT OPTION;
FLUSH PRIVILEGES;
...
printf "%s" "$db_password" > "$INSTALL_DIR/portainer_password"
echo "[OK] Portainer Admin-Passwort gesetzt (User: admin, Passwort: DB-Passwort)."
...
# Backend nach ./app/
BACKEND_INFO=$(sm_get_latest "backend-" "stable")
BACKEND_TAG=$(echo "$BACKEND_INFO" | cut -d'|' -f1)
BACKEND_URL=$(echo "$BACKEND_INFO" | cut -d'|' -f2)
echo "[INFO] Lade Backend $BACKEND_TAG herunter..."
sm_download_and_extract "$BACKEND_URL" "$INSTALL_DIR/app" || exit 1
echo "[OK] Backend $BACKEND_TAG installiert."

# Frontend nach ./app/wwwroot/
FRONTEND_INFO=$(sm_get_latest "frontend-" "stable")
FRONTEND_TAG=$(echo "$FRONTEND_INFO" | cut -d'|' -f1)
FRONTEND_URL=$(echo "$FRONTEND_INFO" | cut -d'|' -f2)
echo "[INFO] Lade Frontend $FRONTEND_TAG herunter..."
sm_download_and_extract "$FRONTEND_URL" "$INSTALL_DIR/app/wwwroot" || exit 1
echo "[OK] Frontend $FRONTEND_TAG installiert."
