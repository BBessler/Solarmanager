...
# Voraussetzungen: curl wird fuer alles benoetigt
if ! command -v curl &> /dev/null; then
    echo "[INFO] Installiere fehlende Voraussetzungen (curl)..."
...
install_if_missing apache2 php libapache2-mod-php mariadb-server php-mysql debconf-utils ufw
...
<VirtualHost *:90>
    ...
    ErrorLog \${APACHE_LOG_DIR}/solarmanager-api-error.log
    CustomLog \${APACHE_LOG_DIR}/solarmanager-api-access.log common
</VirtualHost>
EOF
...
sm_download_and_extract "$BACKEND_URL" "$WEB_DIR/backend" "true" || exit 1
...
sm_download_and_extract "$FRONTEND_URL" "$WEB_DIR" "true" || exit 1
